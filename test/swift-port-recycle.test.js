...
test('Swift DSH service reclaims a stale server bound to its port on startup', () => {
  ...
  assert.match(SERVICE_SOURCE, /record\.entryPath/)
  assert.match(SERVICE_SOURCE, /record\.nodePath/)
  assert.match(SERVICE_SOURCE, /--profile web/)
  assert.match(SERVICE_SOURCE, /--host 127\.0\.0\.1/)
  assert.ok(SERVICE_SOURCE.includes('command.contains("--port \\(record.port)")'))
