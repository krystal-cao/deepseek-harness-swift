...
test('Profile switches are recoverable across force-quit and commit only after a healthy restart', () => {
  ...
  assert.match(SETTINGS_SOURCE, /public func retryPendingProfileSwitchCleanup\(\) async/)
  ...
  assert.match(SETTINGS_SOURCE, /removeDesktopHostArtifacts\(from: \.web\)/)
  ...
  assert.match(SETTINGS_SOURCE, /restartDshServiceDuringOperation\(\)[\s\S]*pendingProfileSwitch = cleanupError == nil \? nil : finalizingTransaction/)
  assert.match(SETTINGS_SOURCE, /restartDshServiceDuringOperation\(\)[\s\S]*pendingProfileSwitch = cleanupError == nil \? nil : transaction/)
...
test('runtime confirmation waits for the rendered Web UI after navigation', () => {
  ...
  assert.match(WINDOW_SOURCE, /restoreWebProfileSnapshot\(snapshotID, profile: snapshotProfile\) \{ progress in/)
  assert.match(WINDOW_SOURCE, /guard appProfile == snapshotProfile else/)
